##  Capítulo Como seleciono um subconjunto de df ? 
O que aprenderemos

- Leitura. (Armazenar todo(s) o(s) documento(s).
- Impressão de dados brutos.
- Impressão da quantidade linhas na coluna selecionada.
- Armazenar colunas específicas do dataframe.
- Armazenar o tipo de dado na coluna selecionada.
- Solicitar quantidades de linhas na coluna selecionada.
- Armazenar múltiplas colunas.
- Armazenar o tipo de dado de múlitplas colunas.
- Armazenar quantidades de linhas e colunas no dataframe.
- 

#

    df = pd.read_#tipo("#local") 
    # Leitura(armazenamento) #tipo_de_arquivo #local

    print(df.head(#num)) 
    # Impressão de dados brutos #num (Quantidade de linhas a serem impressas)

    print(#col.shape)
    # Impressão quantidades de linhas na coluna selecionada #col

    coluna_#col  = df["#col"]  
    # Armazenar colunas específicas do dataframe #col

    tipos_de_valores_em_#col = type(df["#col"])
    # Armazenar o tipo de dado na colunas selecionada. #col

    contagem_de_linhas_na_coluna_#col = df['#col'].shape
    # Armazenar quantidade de linhas na coluna. #col

    selecao_de_colunas_#col_#col = df[["#col", "#col"]]
    # Armazenar multiplas colunas. #col #col

    solicitar_o_tipo_de_dado_contidos_nas_colunas_#col_#col_ = type(df[["#col", "#col"]])
    # Armazenar o tipo de dado de múlitplas colunas. #col

    solicitar_quantidade_de_linhas_e_colunas_em_#col = df[["#", "#"]].shape
    # Armazenar quantidades de linhas e colunas no dataframe. #col #col

    solicitar_em_#col_valores_maiores_que_#num = df[df["#"] > #num]
    # Armazenar valores maiores que #num na coluna #col.

    solicitar_em_#col_valores_maiores_que_#num_saida_em_booleanos = df["#"] > #num]
    # Armazenar valores maiores que #num na coluna #col saída em booleanos. #col #num

    valores_na_coluna_#col_nas_linhas_#lin_e_#lin = titanic[titanic["#"].isin([#, #])]
    # Armazenar valores da coluna #col linha #lin e #lin

    solicitar_valores_nao_nulos_em_#col = df[df["#"].notna()]
    # Armazenar valores não nulos em #col 

    valores_maiores_que_35_na_coluna_#_ao_lado_imprima_coluna_#col = df.loc[df["#"] > 35, "#"]
    # Armazenar valores maiores que #num da coluna #col ao lado imprima coluna #col

    linhas_#lin_a_#lin_e_colunas_#num_a_#num = df.iloc[#lin:#lin, #col:#col]
    # Armazenar linhas: #lin a #lin e colunas #num a #num.

    df.iloc[0:#,#]="anonymous"
    # Atribuir o nome anonymous nos #num primeiro itens da  coluna #num







